...
class Buttons {
    ...
    color;
    direction;
    constructor(color, indexes, direction) {
        this.position = new Vector2(indexes[0] + 0.5, indexes[1] + 0.5);
        this.color = color;
        this.direction = direction || Math.floor(Math.random() * 4);
    }
    updateDirection() {
...
// #endregion
// #region EVENTS
function keyDownEvent(event, ctx) {
...
function mouseDownEvent(event, ctx) {
...
// #endregion
// #region RENDER
function renderButtonText(ctx, button) {
    function renderArrowStrokes(ctx, first, middle, last) {
